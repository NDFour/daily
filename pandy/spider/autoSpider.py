...
class www_8vdy_Spider:
    # 采集网站的目录url
    category_urls = [
            'http://www.8vdy.com/list/index', # 电影第一页
            'http://www.8vdy.com/list/index1_', # 电影后续页码
            'http://www.8vdy.com/list/index', # 电视剧第一页
            'http://www.8vdy.com/list/index2_' # 电视剧后续页码
            ]
    # 每次需要更新的页数+1
    pages_num = 5
    ...
    def __init__(self):
        ...
        str_2_logfile.append('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
        # print('\tmovieSpider for yeyoufang.com')
        str_2_logfile.append('\twww_8vdy_Spider for 8vdy.com')
        # print()
        # print('>> movieSpider init...')
        str_2_logfile.append('\n>> movieSpider init...')
        # 关闭 django 应用 pandy
    ...
    def get_url(self):
        ...
        for category in self.category_urls:
            ...
            while current_page < self.pages_num:
                ...
                # 构造响应页码目录url，并获取目录页网页 文本
                category_html = self.get_html(category + str(current_page) + '.html')
                # 只解析 class="listBox" 标签，其中包含电影名和详情页url
                only_title_href = SoupStrainer(class_="listBox")
                soup = BeautifulSoup(category_html, 'lxml', parse_only=only_title_href)
                ...
                for i in h3_list:
                    a_tmp = i.find('a')
                    a_list_tmp.append('http://www.8vdy.com' + a_tmp['href'])
                    title_list_tmp.append(a_tmp['title'])
                movies_num += len(a_list_tmp)

                i_cnt2 = 0
                for i in a_list_tmp:
                    href = i
                    title = title_list_tmp[i_cnt2]
                    i_cnt2 += 1
                    # 判断是否已经存在于数据库，是的话跳过，不是则存储
                    if is_saved(href) == 1:
                        # print('>> [get_url] skip already exsist\n  %s' % title)
    ...
    def get_info(self, detail_url, movie_name):
        ...
        '''
        # 只解析 第一个 dd 标签
        soup = BeautifulSoup(detail_html, 'lxml').find('dd')
        ...
        try: # 网盘链接
            movie_bdpan = soup.find('li').a['href']
        except:
            movie_bdpan = '#'

        try: # 网盘密码
            movie_pass = soup.find('li').a.string.split(' ',2)[2]
        except:
            movie_pass = ''
...
def main():
    ...
    # 8vdy.com 爬虫
    www_8vdy = www_8vdy_Spider()
    title_list, url_list = www_8vdy.get_url()
    str_2_logfile.append('---------- www_8vdy 共有 %s 条数据需要插入 --------' % len(url_list))
    www_8vdy_cnt_tmp = 0
    for detail_url in url_list:
        www_8vdy.get_info(detail_url, title_list[www_8vdy_cnt_tmp])
        www_8vdy_cnt_tmp += 1
